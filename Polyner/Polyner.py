    ...
    for e in loop_tqdm:
        ...
        if (e + 1) % save_epoch == 0:
            ...
            with torch.no_grad():
                ...
                energy_levels = [int(np.mean(np.arange(0, e_level))), 65, 75, 90, 100]
                output_ids = [img_id, 65, 75, 90, 100]
                
                def generate_image(energy_idx):
                    for i, (xy) in enumerate(test_loader):
                        xy = xy.to(device).float().view(-1, 2)
                        img = network(xy)[:, energy_idx].view((2 * SOD) + 1, (2 * SOD) + 1)
                        img = img.float().cpu().detach().numpy()[kx:kx + h, ky:ky + w]
