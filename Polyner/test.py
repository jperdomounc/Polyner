...
proj_path = "./input/RANDO_no_implants_3mm/LE/proj.nii"
proj = sitk.GetArrayFromImage(sitk.ReadImage(proj_path))
print(f'Original shape: {proj.shape}')  # (360, 744, 229) i.e. (k, j, i)

# Use transpose to convert dimensions from (k, j, i) back to (i, j, k)
# reshape only reinterprets shape without moving data
# transpose actually rearranges the data in memory
proj = proj.transpose(2, 1, 0)  # or np.transpose(proj, (2, 1, 0))
print(f'Transposed shape: {proj.shape}')  # (229, 744, 360)

# Check points (Python is 0-indexed, so subtract 1 from MATLAB indices)
print(f'Point 1: {proj[199, 699, 89]}')   # corresponds to MATLAB (200, 700, 90)
print(f'Point 2: {proj[99, 49, 89]}')     # corresponds to MATLAB (100, 50, 90)
print(f'Point 3: {proj[149, 499, 299]}')  # corresponds to MATLAB (150, 500, 300)
print(f'Point 4: {proj[219, 399, 299]}')  # corresponds to MATLAB (220, 400, 300)


# ## `reshape` vs `transpose`

# Original (2x3):        reshape(3,2):          transpose:
# [[1, 2, 3],           [[1, 2],               [[1, 4],
#  [4, 5, 6]]            [3, 4],                [2, 5],
#                        [5, 6]]                [3, 6]]
                       
# reshape just regroups   transpose actually swaps rows/cols
        ...
        if (e + 1) % save_epoch == 0:
            ...
            with torch.no_grad():
                ...
                for i, (xyz) in enumerate(test_loader):
                    ...
                    # Process points in chunks to fit in GPU memory
                    ...
                    for start_idx in range(0, num_points, chunk_size):
                        ...
                        # Network output: (chunk_size, e_level), select middle energy
                        chunk_out = network(xyz_chunk)[:, energy_idx]
                        img_pre_list.append(chunk_out.cpu())
                    
                    # Concatenate all chunks and reshape to volume
